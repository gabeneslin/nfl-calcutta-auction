...
exports.populateTeams = functions.https.onCall(async (data, context) => {
  ...
  // Reset auction state to first team
  const auctionRef = db.collection("settings").doc("auction");
  batch.set(auctionRef, {
    currentTeamId: shuffledTeamIds[0],
    teamOrder: shuffledTeamIds,
    highBid: 0,
    highBidder: "",
  });
  ...
  return {success: true, message: "Teams and auction state reset."};
});
